...
# func to identify circuit for refusal, hoping to isolate eval from action.
def circuit():
    pass 

def run_diy_refuse(model_path):

    # call orthogonalized model (w/ modification: adding refusal behavior rather than removing it)
    model_alias = os.path.basename(model_path)
    cfg = Config(model_alias=model_alias, model_path=model_path)

    # The code actually calls the reconstructed model 
    model_base = construct_model_base(cfg.model_path)
    # NEP not sure whether this is modified as intended already 
    # dataset: numerous refusal cases with diff human-interpretable semantic meanings 
    
    # We could try on truthfulQA first 
    # a simple way might be to test the newly orthogonalized model on TruthfulQA dataset 

    
    # to identify the safety-refusal circuit 




    # seperate model behaviors: like can you let the model realize that they should refuse but they don't take actions?
    # but it's just one vector. How can you diff it into maybe two?
...
def run_pipeline(model_path):
    ...
    # NEP Here we can add DIY dataset
    ...
    '''
    NEP Mar 8 - comment out bc together AI rate limit. Will focus on other part first. 
